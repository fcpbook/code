...
object Section_6:
   ...
   def get[A](zipper: Zipper[A]): A = (zipper: @unchecked) match
      case (_, x :: _) => x

   def set[A](zipper: Zipper[A], value: A): Zipper[A] = (zipper: @unchecked) match
      case (left, _ :: right) => (left, value :: right)

   def moveLeft[A](zipper: Zipper[A]): Zipper[A] = zipper match
